...
namespace WebApi.Controllers
{
    ...
    public class TasksController : ControllerBase
    {
        ...
        [ProducesResponseType(typeof(UpdateTaskCommandResult), StatusCodes.Status200OK)]
        public async Task<IActionResult> Assign(Guid taskId, Guid memberId)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            }
            UpdateTaskCommand command = (await _taskService.GetTaskQueryHandler(taskId)).ToUpdateTaskCommand();
